...
While there are various ways to filter cells, here we will just use the barcode ranks knee calculation for the time being. We subset on the columns those that have more counts than our calculated knee.

```{r}
sce <- sce[, sce$log10_total_counts > knee]
```

We end up keeping `r ncol(sce)` cells for subsequent steps of the analysis.
...
We see above that we now have two assays, `counts` and `logcounts`. 
...
Let's first read in the marker matrix from the paper's associated [Github](https://github.com/ebecht/mcpcounter):

```{r}
anno <- read.table('https://raw.githubusercontent.com/ebecht/MCPcounter/master/Signatures/genes.txt')
colnames(anno) <- c('SYMBOL', 'cell_type', 'ENTREZID')
anno <- anno[, -3]

anno
```

Based on our manual inspection, we can take this annotation and filter down to our primary cell types of interest.

```{r}
## filter down to predominant cell types of interest to increase signal
ci <- c('T cells', 'B lineage', 'NK cells', 'Myeloid dendritic cells')
anno <- anno[(anno$cell_type %in% ci), ]
```
...
We can see that `cellassign` has produced cell type labels in a manner largely consistent with our expectations based on manual inspection. 
